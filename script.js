...
// 建物ごとにデータを絞り込んで表示します。
//
// ・INSPECTION_BASIC_CSV_URL     基本情報：建物名,オーナー様,管理営業所,担当者,建物点検日,建物CD,建物完成日,契約形態,建物写真URL
...
function openPdfViewer(url) {
  ...
  frame.src = toEmbeddablePdfUrl(url);
...
async function loadPayments() {
  ...
  try {
    ...
    if (!idToken) {
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="3" style="text-align:center">ログイン情報が確認できませんでした（idTokenを取得できません）</td></tr>`;
      }
      return;
    }
    ...
    // Apps Script側がエラーを返した場合（認証エラーなど）はそのまま画面に表示する
    if (json && json.error) {
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="3" style="text-align:center">Apps Scriptからのエラー：${
          json.error
        }${
          json.debug
            ? `<br><span style="font-size:12px;color:#999;">詳細：${json.debug}</span>`
            : ""
        }</td></tr>`;
      }
      return;
    ...
    if (!Array.isArray(json)) {
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="3" style="text-align:center">想定外のデータ形式で受信しました：${JSON.stringify(
          json
        ).slice(0, 200)}</td></tr>`;
      }
      return;
    ...
    if (tbody) {
      tbody.innerHTML = `<tr><td colspan="3" style="text-align:center">読み込みエラー：${e.message}</td></tr>`;
    }
    console.error(e);
  }
}
