...
async function preloadAllData() {
  // まず認証付きAPI（お支払い以外）のデータを1回のリクエストでまとめて取得しておく。
  // こうすることで、ログイン確認の通信・スプレッドシートを開く処理が
  // データ種別の数だけ繰り返されるのを防ぎ、大幅に速くなる。
  try {
    __batchCsvCache = await fetchSecureCsvBatch([
      "notices",
      "recruit",
      "documents",
      "repair",
    ...
    console.error(
      "まとめてのデータ取得に失敗しました。個別に取得し直します：",
      e
    );
    __batchCsvCache = null; // 失敗時は、以下の各処理がこれまで通り個別に取得する
  }
  ...
  // まとめ取得の結果は、この初回読み込みだけで使い切る
  // （以降、個別に再取得したい場面では通常通りサーバーに問い合わせさせる）
  __batchCsvCache = null;

  results.forEach((r) => {
    if (r.status === "rejected") {
      console.error("先読みに失敗した項目があります：", r.reason);
    }
  });
}
