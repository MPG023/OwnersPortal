...
let readNotices = [];
let allNoticeRows = [];
let currentNoticeYear = new Date().getFullYear();
let noticeYearInitialized = false;
...
function getNoticeYear(date) {
  const yearMatch = String(date || "").match(/(?:^|\D)(\d{4})(?=\D|$)/);
...
function getNoticeYearBounds() {
  const availableYears = allNoticeRows
    .map((cols) => getNoticeYear(cols[4]))
    .filter((year) => year !== null);
  const thisYear = new Date().getFullYear();

  return {
    minYear: Math.min(thisYear - 1, ...availableYears),
    maxYear: Math.max(thisYear, ...availableYears)
  };
}

function renderNoticeYearControls() {
  const yearEl = document.getElementById("noticeYear");
  const prevButton = document.getElementById("prevNoticeYearBtn");
  const nextButton = document.getElementById("nextNoticeYearBtn");
  const yearGrid = document.getElementById("noticeYearPickerGrid");
  if (!yearEl || !prevButton || !nextButton || !yearGrid) return;

  const { minYear, maxYear } = getNoticeYearBounds();
  currentNoticeYear = Math.max(minYear, Math.min(currentNoticeYear, maxYear));

  yearEl.textContent = `${currentNoticeYear}年`;
  prevButton.disabled = currentNoticeYear <= minYear;
  nextButton.disabled = currentNoticeYear >= maxYear;

  let html = "";
  for (let year = minYear; year <= maxYear; year++) {
    const selected = year === currentNoticeYear;
    html += `<button type="button" class="year-picker-option${
      selected ? " selected" : ""
    }" ${selected ? 'aria-current="true"' : ""} onclick="selectNoticeYear(${year})">${year}年</button>`;
  }
...
function renderNoticeList() {
  ...
  renderNoticeYearControls();
  const visibleRows = allNoticeRows.filter((cols) => {
    const year = getNoticeYear(cols[4]);
    return (
      year === currentNoticeYear ||
      (year === null && currentNoticeYear === new Date().getFullYear())
    );
  });

  list.innerHTML = "";
  if (visibleRows.length === 0) {
    list.textContent = "この年のお知らせはありません。";
    return;
  }
  ...
  visibleRows.forEach((cols) => {
    ...
    const isLink =
      !!file &&
      (normalizedFile.startsWith("http") || normalizedFile.startsWith("www."));
...
function openNoticeYearPicker() {
  renderNoticeYearControls();
  const modal = document.getElementById("noticeYearPickerModal");
  if (modal) modal.classList.add("open");
}

function closeNoticeYearPicker() {
  const modal = document.getElementById("noticeYearPickerModal");
...
document.addEventListener("keydown", (event) => {
  ...
  closeNoticeYearPicker();
  closeMonthPicker();
});

function changeNoticeYear(diff) {
  const { minYear, maxYear } = getNoticeYearBounds();
  const nextYear = currentNoticeYear + diff;
  if (nextYear < minYear || nextYear > maxYear) return;

  currentNoticeYear = nextYear;
  renderNoticeList();
}

function selectNoticeYear(year) {
  const { minYear, maxYear } = getNoticeYearBounds();
  if (!Number.isInteger(year) || year < minYear || year > maxYear) return;

  currentNoticeYear = year;
  closeNoticeYearPicker();
  renderNoticeList();
}
...
async function loadNotices() {
  ...
  if (!list.children.length && !allNoticeRows.length) {
  ...
  try {
    ...
    if (!noticeYearInitialized) {
      const availableYears = rows
        .map((cols) => getNoticeYear(cols[4]))
        .filter((year) => year !== null)
        .sort((a, b) => a - b);
      const thisYear = new Date().getFullYear();
      const currentYearRows = availableYears.includes(thisYear);
      const pastYears = availableYears.filter((year) => year <= thisYear);

      currentNoticeYear = currentYearRows
        ? thisYear
        : pastYears.length
        ? pastYears[pastYears.length - 1]
        : thisYear;
      noticeYearInitialized = true;
    }
...
function jumpToLatestAvailableMonth() {
  ...
  const availableMonths = allPayments
    ...
    .filter((month) => {
      ...
      return isSelectablePaymentMonth(year, monthNumber, range);
    })
    .sort();
...
function updateButtons() {
  ...
  const minMonthIndex = range.minYear * 12 + range.minMonth - 1;
  const maxMonthIndex = range.maxYear * 12 + range.maxMonth - 1;
  const prevButton = document.getElementById("prevMonthBtn");
  const nextButton = document.getElementById("nextMonthBtn");

  if (prevButton) prevButton.disabled = currentMonthIndex <= minMonthIndex;
  if (nextButton) nextButton.disabled = currentMonthIndex >= maxMonthIndex;
}
...
document.getElementById("prevMonthBtn").onclick = () => {
  const range = getSelectableRange();
  if (isSelectablePaymentMonth(currentYear, currentMonth - 1, range)) {
    currentMonth--;
    if (currentMonth === 0) {
      currentMonth = 12;
      currentYear--;
...
function openMonthPicker() {
  const range = getSelectableRange();
  pickerYear = Math.max(range.minYear, Math.min(currentYear, range.maxYear));
  renderMonthPicker();
...
function changePickerYear(diff) {
  const range = getSelectableRange();
  const nextYear = pickerYear + diff;
  if (nextYear < range.minYear || nextYear > range.maxYear) return;
...
function getSelectableRange() {
  ...
  return {
    minYear: 2026,
    minMonth: 1,
    maxYear: nextMonth.getFullYear(),
    maxMonth: nextMonth.getMonth() + 1
  };
}

function isSelectablePaymentMonth(year, month, range = getSelectableRange()) {
  if (month < 1) {
    year--;
    month = 12;
  } else if (month > 12) {
...
function renderMonthPicker() {
  ...
  const range = getSelectableRange();
  const prevYearButton = document.getElementById("monthPickerPrevYearBtn");
  const nextYearButton = document.getElementById("monthPickerNextYearBtn");
  if (prevYearButton) prevYearButton.disabled = pickerYear <= range.minYear;
  if (nextYearButton) nextYearButton.disabled = pickerYear >= range.maxYear;
...
function selectPickerMonth(m) {
  if (!isSelectablePaymentMonth(pickerYear, m)) return;
