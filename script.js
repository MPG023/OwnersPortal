...
async function decodeResponseText(response) {
  ...
  if (utf8Text.includes("\uFFFD") || /繧|縺|譁|蜿|莠|闔/.test(utf8Text)) {
    try {
      return new TextDecoder("shift-jis").decode(bytes);
    } catch (error) {
      console.error("CSVの文字コード判定に失敗しました。UTF-8で処理します。", error);
    }
  }
...
let loadErrorPageLocked = false;
let loadingTimeoutTimer = null;
const LOADING_ERROR_TIMEOUT_MS = 5000;

function showLoadingOverlay() {
  if (loadErrorPageLocked) return;
  ...
  loadingTimeoutTimer = setTimeout(handleLoadingTimeout, LOADING_ERROR_TIMEOUT_MS);
}
...
function hideLoadingOverlay() {
  ...
  const overlay = document.getElementById("loadingOverlay");
  if (overlay && !loadErrorPageLocked) {
...
function handleLoadingTimeout() {
  ...
  const loadErrorPage = document.getElementById("loadErrorPage");

  if (overlay) overlay.style.display = "none";
  if (portal) portal.style.display = "none";
  if (loginPage) loginPage.style.display = "none";
  if (loadErrorPage) loadErrorPage.style.display = "flex";

  localStorage.removeItem("loggedIn");
  auth.signOut().catch((error) => {
    console.error("読み込みエラー画面への切り替え時のログアウトに失敗しました。", error);
  });
}

function handleInitialLoadTimeout() {
  handleLoadingTimeout();
}

async function returnToLoginFromLoadError() {
  if (!loadErrorPageLocked) return;
  ...
  localStorage.removeItem("loggedIn");
  loadErrorPageLocked = false;

  const loadErrorPage = document.getElementById("loadErrorPage");
  const loginPage = document.getElementById("loginPage");
  if (loadErrorPage) loadErrorPage.style.display = "none";
...
["click", "mousedown", "mouseup", "pointerdown", "pointerup", "touchstart", "touchend", "wheel", "scroll", "contextmenu"].forEach(
  (eventName) => {
    document.addEventListener(
      eventName,
      (event) => {
        if (!timeoutPageLocked && !loadErrorPageLocked) return;

        const timeoutPage = document.getElementById("timeoutPage");
        const loadErrorPage = document.getElementById("loadErrorPage");
        const activePage =
          loadErrorPageLocked && loadErrorPage && loadErrorPage.style.display !== "none"
            ? loadErrorPage
            : timeoutPage;
        if (!activePage || activePage.style.display === "none") return;

        const target = event.target;
        const loginButton =
          target && target.closest ? target.closest(".timeout-btn") : null;

        const loadErrorButton =
          target && target.closest ? target.closest(".load-error-btn") : null;

        if (
          (loginButton && timeoutPage && timeoutPage.contains(loginButton)) ||
          (loadErrorButton && activePage.contains(loadErrorButton))
        ) {
          return;
        }
  ...
  (event) => {
    ...
    const allowedButton =
      target && target.closest
        ? target.closest(".timeout-btn, .load-error-btn")
        : null;
    if (allowedButton && activePage.contains(allowedButton)) return;
...
document.addEventListener("DOMContentLoaded", () => {
  if (
    window.__siteLoadState &&
    window.__siteLoadState.timedOut
  ) {
    handleInitialLoadTimeout();
  }
  ...
  window.addEventListener("site-load-timeout", handleInitialLoadTimeout);
...
firebase.auth().onAuthStateChanged(async (user) => {
  if (user) {
    ...
    if (timeoutPageLocked || loadErrorPageLocked) {
      document.getElementById("portal").style.display = "none";
      document.getElementById("loginPage").style.display = "none";
      return;
    }
    ...
    if (!noticeRefreshTimer) {
      noticeRefreshTimer = setInterval(loadNotices, 30000);
    }
    backHome();
    resetLogoutTimer();
  } else {
...
function loadNoticeCount(count) {
  const badgeIds = ["noticeCount", "noticeCountHome", "noticeCountMenu"];
  ...
  const panelBadge = document.getElementById("noticeCountMenuPanel");
  if (panelBadge) {
    panelBadge.style.display = count > 0 ? "inline-flex" : "none";
    panelBadge.textContent = count > 0 ? `新着${count}件` : "";
  }
}
...
async function loadNotices() {
  if (noticeLoadPromise) return noticeLoadPromise;

  const list = document.getElementById("noticeList");
  if (!list) return;

  noticeLoadPromise = (async () => {
